 ...
 *                                                                           
 * @author Fawzi Emad, Daniel Arefian
 */
public class Model {
	...
	private boolean isSpaceAvailable(int r, int c) {
			boolean fishThere = false;
			boolean plantThere = false;
			
		
		for(Fish x:fish) { 
			if(x.getCol()==c&&x.getRow()==r) { 
				 fishThere= true; 
				 //if fish is there get out of loop
				 break;
			} 
		} 
		for(Plant y:plants) { 
			if(y.getCol()==c&&y.getRow()==r) { 
				plantThere = true; 
				//if plant there get out of loop
				break;
			} 
		} 
		 
		if(plantThere||fishThere||landscape[r][c] == Model.ROCK) {
			return false;
		}else {
			return true;
		}
	...
	public Model(Model other) {
		...
		this.landscape=other.landscape; //shallow 
		this.fish = new ArrayList <Fish>(); 
		for(Fish x: other.fish) { 
			this.fish.add(new Fish(x)); 
		} 
		this.plants = new ArrayList <Plant>(); 
		for(Plant y: other.plants) { 
			this.plants.add(new Plant(y)); 
		} 
	...
	public static void fishEatPlant(Fish f, Plant p) {
		...
		int smallerSize = (Plant.PLANT_BITE_SIZE>=p.getSize()?p.getSize():Plant.PLANT_BITE_SIZE); 
		f.eat(smallerSize); 
		p.removeBite(smallerSize); 
	...
	public void growPlants() {
		ArrayList<Plant> plantsList = new ArrayList <Plant>(this.plants); 
		for(Plant x : plantsList) { 
			x.grow(); 
		} 
	} 
	...
	public void removeDeadFish() {
		ArrayList <Fish> newList = new ArrayList <Fish>(this.fish); 
		for(Fish x : newList) { 
			if(!(x.isAlive())) { 
				this.fish.remove(x); 
			} 
	...
	private boolean fishIsSurroundedByRocks(Fish f) {
		...
		int counter =0; 
		if(landscape[f.getRow()+1][f.getCol()] == ROCK) { 
			counter++; 
		}
		if(landscape[f.getRow()-1][f.getCol()] == ROCK) { 
			counter++; 
	...
	public void turnFish() {
		for(Fish x: this.fish) { 
				if(this.fishIsSurroundedByRocks(x)) { 
					continue; //if dish is surrounded by rocks, skip current fish 
				} if((x.getDirection() != Fish.UP)&&(x.getDirection() != Fish.DOWN) 
						&&(x.getDirection() != Fish.LEFT)&&(x.getDirection() != Fish.RIGHT)){ 
					throw new IllegalFishDirectionException(x.getDirection()); 

				} else {
					//switch the immediate direction of the fish 
					int row=x.getRow(); 
					int col = x.getCol();
					if(x.getDirection()==Fish.UP){ 
						...
						row++; 
					}else if(x.getDirection()==Fish.LEFT){ 
						col--; 
					}else if(x.getDirection()==Fish.RIGHT){ 
						col++; 
					} 
					
					while(landscape[row][col]==ROCK&&!fishIsSurroundedByRocks(x)){ 
	...
	public void moveFish() {
		for(Fish x: this.fish) { 
			if(x.isAlive()) { 
				if(this.fishIsSurroundedByRocks(x)) {  
					continue; //if fish is surrounded by rocks, skip current fish 
				} 
				x.move();
				for(Plant p:this.plants) { 
					if(p.isAlive()&&p.getCol()==x.getCol()&&p.getRow()==x.getRow()) { 
						fishEatPlant(x,p); //if fish is on top of plant, eats it
					} 
				} 


				for(Fish secondFish:this.fish) { 
					//make sure current fish does not equal other fish
					if(!(x.equals(secondFish))&&secondFish.getCol()==x.getCol()&& 
							secondFish.getRow()==x.getRow()) { 
						x.fight(secondFish); //1v1 if fishes are on same spot
					} 
				} 
	...
	public void addPlant(Plant p) {
		if(this.landscape[p.getRow()][p.getCol()]==Model.ROCK) {  
			//if plants location is going to be on the same place as a rock 
			throw new IllegalPlantPositionException 
			(IllegalPlantPositionException.PLANT_OVER_ROCK); 
		} 

		for(Plant p2:plants) { 
			if(p2.getCol()==p.getCol()&&p2.getRow()==p.getRow()&&(!(p2==p))) { 
				//if a plant is there: 
				throw new IllegalPlantPositionException 
				(IllegalPlantPositionException.TWO_PLANTS_IN_ONE_PLACE); 
			} 

			//if we are in legal position
		}this.plants.add(p); 
	}
	...
	public void addFish(Fish f){
		...
		for(int i=0;i<fish.size();i++){ 
			//go through each index manually
	...
	public boolean getShape(int row, int col) {
		return(this.landscape[row][col] == Model.ROCK?Model.ROCK:Model.WATER);
